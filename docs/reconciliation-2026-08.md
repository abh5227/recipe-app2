...
| # | contradiction | risk |
| --- | --- | --- |
| **D1** | ✅ **CLOSED BY DELETION (migration 046, 2026-09-20).** The contradiction was between `CLAUDE.md:349` calling the seed-rebuild intended and `docs/panel-design.md` deciding the 36 should become durable rows. ⚠️ Neither answer survived. The 36 were an early demo of what an ingredient library would look like, and they were deleted rather than made durable. `seed.py`'s `INGREDIENTS` is empty, `ingredients` holds 0 rows, and both documents now say so. | done |
...
| D3 | ✅ **CLOSED BY DELETION (migration 046, 2026-09-20).** The four competing names for the same 36 rows ("the owner's corpus", "the seed 'bones'", "the 36 library descriptions", "a shared ingredient field guide") no longer name anything. The rows are gone. ⚠️ The term that needed propagating turned out to be the wrong question, and the surviving vocabulary is one word, **library**, meaning the 10,013-row catalog. `README`, `OVERVIEW`, `models.py` and `seed.py` were updated in the same pass. | done |
