...
Guards the decisions that are expensive to reverse later:
  ...
  - licence and attribution have a column and are filled BEFORE data lands;
...
def test_six_sources_seven_datasets(db):
    """USDA Foundation and SR Legacy share the FDC schema, so six sources, seven datasets."""
    rows = db.execute("SELECT source, dataset FROM source_catalogue").fetchall()
    assert len(rows) == 7
    assert len({r["source"] for r in rows}) == 6
    usda = [r["dataset"] for r in rows if r["source"] == "usda_fdc"]
    assert sorted(usda) == ["foundation_foods", "sr_legacy"]
    # ...and they share ONE entry table, told apart by source_fetch.dataset.
    assert "usda_fdc_entry" in tables(db)
    assert "sr_legacy_entry" not in tables(db)
    assert "foundation_foods_entry" not in tables(db)
...
def test_licence_and_attribution_have_a_column_and_are_filled(db):
    """The precedent this schema exists to avoid: attribution with nowhere to go."""
    for t in ("source_catalogue", "source_fetch"):
        assert {"licence", "attribution"} <= cols(db, t)
    rows = db.execute("SELECT source, dataset, licence, attribution FROM source_catalogue").fetchall()
    assert rows, "catalogue is empty, so attribution landed nowhere"
    for r in rows:
        assert r["licence"].strip(), f"{r['source']}/{r['dataset']} has no licence"
...
def test_wikidata_query_is_not_english_only(db):
    ...
    assert 'LANG(?text) = "en"' not in q
    assert "?lang" in q, "the query must return the language of each label"


def test_agrovoc_query_is_english_only_for_licence_reasons(db):
    r = db.execute(
        "SELECT query_text, notes FROM source_catalogue WHERE source='agrovoc'"
    ).fetchone()
    assert 'LANG(?text) = "en"' in r["query_text"]
    assert "6 of" in r["notes"], "the 6-of-42-languages restriction must be recorded"
...
def test_fetch_requires_a_catalogued_dataset(db):
    with pytest.raises(sqlite3.IntegrityError):
        db.execute(
            ...
            "(source, dataset, url, licence, attribution, fetched_at, bytes, sha256) "
            "VALUES (?,?,?,?,?,?,?,?)",
            ("nope", "nope", "https://x", "CC0", "x", "2026-08-23T00:00:00Z", 1, "abc"),
        )
...
def test_native_label_without_english_is_queryable(db):
    ...
    db.execute(
        "INSERT INTO source_fetch "
        "(source, dataset, url, licence, attribution, fetched_at, bytes, sha256) "
        "VALUES ('wikidata','food_items_q2095','https://query.wikidata.org/sparql',"
        "'CC0-1.0','Wikidata contributors','2026-08-23T00:00:00Z',1,'abc')"
    )
...
def test_label_kind_is_not_normalised(db):
    ...
    db.execute(
        "INSERT INTO source_fetch "
        "(source, dataset, url, licence, attribution, fetched_at, bytes, sha256) "
        "VALUES ('foodon','owl_release','http://purl.obolibrary.org/obo/foodon.owl',"
        "'CC-BY-3.0','FoodOn','2026-08-23T00:00:00Z',1,'abc')"
    )
    fid = db.execute("SELECT id FROM source_fetch").fetchone()[0]
    db.execute("INSERT INTO foodon_entry (fetch_id, entry_id, name, raw) VALUES (?,?,?,?)",
               (fid, "FOODON_00001", "x", "{}"))
    pk = db.execute("SELECT pk FROM foodon_entry").fetchone()[0]
    for k in kinds:
        db.execute("INSERT INTO foodon_label (entry_pk, text, kind) VALUES (?,?,?)", (pk, "t", k))
    got = {r[0] for r in db.execute("SELECT DISTINCT kind FROM foodon_label")}
