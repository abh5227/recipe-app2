...
# The lookup build_db's seed_library_names loads. Gitignored and server-side, like the two
# databases it is derived from. See write_library_names.
NAMES_OUT = os.environ.get("LIBRARY_NAMES_CSV", "library_names.csv")
...
def write_library_names(rows, path):
    ...
    KEPT ROWS ONLY, and `not row["cut_by"]` is the same predicate write_sheet uses for its own kept
    ...
    ⚠️ A DUPLICATE ID STOPS THE BUILD HERE RATHER THAN AT LOAD TIME. library_names.library_id is a
    ...
    """
    kept = sorted((r for r in rows if not r["cut_by"]), key=lambda r: r["canonical"].casefold())
    seen = {}
    for row in kept:
        if row["id"] in seen:
            raise SystemExit(
                f"⚠️  DUPLICATE library id {row['id']!r}: '{seen[row['id']]}' and "
                f"'{row['canonical']}'. library_names.library_id is a primary key, so this file "
                "would not load. Fix the rowset rather than dropping one of them here.")
        seen[row["id"]] = row["canonical"]
    with open(path, "w", newline="", encoding="utf-8") as f:
        f.write("# library_names.csv: the ingredient library's id -> canonical-name lookup.\n")
...
def build(join_db=JOIN_DB, sources_db=SOURCES_DB, out=OUT, names_out=NAMES_OUT, verbose=True):
    def say(*a):
        if verbose:
            print(*a, flush=True)
    ...
    # ⚠️ BEFORE write_sheet, and the order is load-bearing. write_sheet needs openpyxl, which is not
    #    in requirements.txt (it imports it inside itself), so a machine without it loses the sheet.
    #    The lookup is the artifact the app actually consumes and it needs nothing but csv, so it
    #    lands first. AFTER guard_unharvested, which raises on unharvested marks: a build that is not
    #    allowed to overwrite the sheet is not allowed to publish a lookup either.
    n_names = write_library_names(rows, names_out)
    say(f"\nwrote {names_out}")
    say(f"  library-name lookup      {n_names:6,d}")
