...
class RecipeSnapshot(Base):
    ...
    028). Two reasons exist and no others: 'original', a birth baseline written by snapshot_original, and
    'cook', written when a cook is logged. A third, 'manual', was designed and then DROPPED (see
    docs/design-decisions.md, change-tracking). The Cooking Journal's foundation (HYBRID): snapshots are the
    STORED TRUTH; diffs are DERIVED from consecutive snapshots (stage 3, shipped) and would be materialized
    for note-linkage later (stage 4). Snapshots ARE read, by _recipe_annotations. cook_log_id -> cook_log ON
    DELETE CASCADE (undo a cook -> its snapshot goes; NULL on an 'original' baseline); recipe_id -> recipes
    ON DELETE CASCADE. user_id is the actor (reference FK, no cascade). content =
    serialize_recipe_content()'s JSON. created_at = now_utc()
    ...
    cook_log_id = Column(Integer, ForeignKey("cook_log.id", ondelete="CASCADE"))              # the cook (NULL on an 'original')
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)                         # who triggered it (no cascade)
    reason = Column(Text, nullable=False)                                                     # 'original' | 'cook'
